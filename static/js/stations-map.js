...
d3.json("http://127.0.0.1:5000/api/v1.0/price_apr21_2020").then(function(data) {
  ...
  for (var index = 0; index < data.data.length; index++) {
    if (data.data[index].Entidad == state){
      var stationMarker = L.marker([data.data[index].Latitud, data.data[index].Longitud])
                            .bindPopup("<h3>" + data.data[index].Razon_Social + "<h3><h3>Franquicia: " 
                            + data.data[index].Franquicia_Marca + "<h3><hr><h3>Precios: </h3><p>Magna: " 
  ...
  function createMap(stationMarkers) {
      ...
      // Create the map object with options
        var map = L.map("map", {
          ...
          zoom: 10,
          layers: [streetmap, stationMarkers]
          });
